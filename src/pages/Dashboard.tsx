...
const Dashboard = () => {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-800'} bg-cover bg-center`}>
      <DiaryNav />
      <div className={contentClass}>
        <header className="mb-10 md:mb-12 text-center">
          <h1 className="text-4xl md:text-5xl font-serif font-medium">
            {getGreeting()}, <span className="text-diary-purple">{profile?.username || "there"}</span> 👋
          </h1>
        ...
        {/* Quick Actions, Recent Entries, Mood Chart */}
        <div className="space-y-6">
          ...
          {/* Below Quick Actions: Recent Entries and Mood Chart side by side */}
      ...
      {/* Dark mode toggle */}
      <Button
        className="fixed bottom-4 left-4 z-40 bg-white text-diary-purple hover:bg-white/90 rounded-full w-10 h-10 p-0"
      ...
      <Button className="fixed top-4 right-4 md:right-6 z-40 bg-white text-diary-purple hover:bg-white/90 rounded-full w-10 h-10 p-0" onClick={() => setShowProfile(true)}>
        <User size={20} />
      </Button>

      {/* Profile modal (avatar & email removed) */}
      {showProfile && (
